...
from src.utils import invoke_with_retry, invoke_agent_text
...
if st.session_state.running and not st.session_state.done:
    ...
    # Groq free tier: ~7000 input tok/min (ITPM) and ~1000 output tok/min (OTPM).
    # Cap every blob feeding a model and route calls through invoke_with_retry,
    # which waits out the per-minute window on a 429/413.
    ...
    with st.spinner("🔍 Search Agent is working…"):
        ...
        search_text = invoke_agent_text(
            ...
            label="search agent",
            fallback=f"(search failed) topic: {topic_val}",
        )

        results["search"] = search_text[:MAX_SEARCH_CHARS]
    ...
    with st.spinner("📄 Reader Agent is scraping top resources…"):
        ...
        results["reader"] = reader_text[:MAX_SCRAPED_CHARS]
        st.session_state.results = dict(results)
